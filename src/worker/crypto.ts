...
export const b64 = (input: Uint8Array) =>
...
async function codeKey(secret: string) {
  const material = await crypto.subtle.digest(
    "SHA-256",
    enc.encode(`upload-grant-code:${secret}`),
  )
...
export async function sealCode(value: string, secret: string) {
  ...
  const ciphertext = await crypto.subtle.encrypt(
    { name: "AES-GCM", iv, additionalData: enc.encode("upload-grant-code:v1") },
    await codeKey(secret),
    enc.encode(value),
  )
...
export async function revealCode(value: string, secret: string) {
  ...
  if (version !== "v1" || !ivValue || !ciphertextValue)
    throw new Error("invalid_code_ciphertext")
  const plaintext = await crypto.subtle.decrypt(
    {
      name: "AES-GCM",
      iv: fromB64(ivValue),
      additionalData: enc.encode("upload-grant-code:v1"),
    },
    await codeKey(secret),
    fromB64(ciphertextValue),
  )
