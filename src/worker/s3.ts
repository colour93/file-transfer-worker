...
export const presignGet = (env: Env, key: string, filename: string, ttl: number, disposition: 'attachment' | 'inline' = 'attachment') => { const url = new URL(urlFor(env, key)); url.searchParams.set('response-content-disposition', `${disposition}; filename*=UTF-8''${encodeURIComponent(filename)}`); url.searchParams.set('X-Amz-Expires', String(ttl)); return client(env).sign(new Request(url, { method: 'GET' }), { aws: { signQuery: true } }).then((r) => r.url) }
