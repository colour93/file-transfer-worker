...
export function previewKind(file: PreviewFile): PreviewKind | null {
  ...
  return (
    (Object.entries(extensions) as Array<[PreviewKind, Set<string>]>).find(
      ([, values]) => values.has(extension),
    )?.[0] || null
  )
}
        ...
        >
          <header className="flex h-16 shrink-0 items-center gap-3 border-b px-3 sm:px-5">
            ...
            {kind === "text" ? (
              <Select
                value={encoding}
                onValueChange={(value) => setEncoding(value as TextEncoding)}
              >
                <SelectTrigger size="sm" aria-label="文本编码">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          >
            ...
            {kind === "text" ? (
              <div className="h-full w-full max-w-5xl overflow-auto border bg-background p-4 sm:p-6">
                {loading ? (
